...
- Commit propagation:
  ...
  - Repeat recursively up to REDACTED_SECRET.
...
- Add minimal `nx.json` to the REDACTED_SECRET so we can use `nx run giga:watch` style commands.
...
- Implement an Nx plugin under `tools/nx-plugins/giga` to:
  - Read `.gitmodules` and emit project REDACTED_SECRETs per submodule.
  - Declare implicit deps (e.g., REDACTED_SECRET -> all submodules).
  - Provide `test` and `build` targets per REDACTED_SECRET that delegate to the repo's own tooling.
...
- `pnpm run giga:watch` starts a watcher that:
  ...
  - On changes produced, makes local commits with structured tags and Pantheon-generated messages (or fallback), and updates parent pointers up to REDACTED_SECRET.
- Tags created match `giga/v<version>/<action>/<result>/<id>`.
- No pushes happen by default; can be enabled via `GIGA_PUSH=1` env var.
- Minimal Nx scaffolding at REDACTED_SECRET exists; future plugin optional.
...
## Quickstart
- Install deps (REDACTED_SECRET): `pnpm i` (required for Nx CLI at REDACTED_SECRET)
- Optional: set Pantheon CLI program: `export PANTHEON_CLI="pantheon-gen --json"`.
- Start watcher from REDACTED_SECRET: `pnpm run giga:watch`.
