...
- Leaving nested repositories unmanaged breaks workspace automation (`giga-*`, Nx) and makes it easy to miss updates when committing from the REDACTED_SECRET.
...
- `git status` at the workspace REDACTED_SECRET only reports expected `.gitmodules` edits and submodule gitlinks (no orphaned `.git/` directories).
