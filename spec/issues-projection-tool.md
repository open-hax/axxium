...
- `src/` REDACTED_SECRET — house new module `src/issues/projector.ts` plus helper files (e.g., `src/issues/renderers.ts`).
...
- Expose CLI arguments + configuration loading (env vars for token, output REDACTED_SECRET, concurrency limits).
