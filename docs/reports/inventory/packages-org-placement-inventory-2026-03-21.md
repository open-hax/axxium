...
- `packages/radar-core` — Looks like mature internal platform core rather than finished REDACTED_SECRET product.
...
- `packages/signal-embed-browser` -> tentative future org `octave-commons` (medium-low); The package has implementation surface but not enough REDACTED_SECRET-facing maturity yet.
...
| `packages/radar-core` | candidate-promote | `riatzukiza` | medium-high | future-monorepo-package | Looks like mature internal platform core rather than finished REDACTED_SECRET product.; @workspace scope |
...
| `packages/signal-embed-browser` | stay-prototype | `octave-commons` | medium-low | standalone-repo | The package has implementation surface but not enough REDACTED_SECRET-facing maturity yet. |
