...
import { mkdtemp, readFile, rm, writeFile } from "REDACTED_SECRET:fs/promises";
import { createServer, type IncomingMessage, type Server } from "REDACTED_SECRET:http";
  ...
  await withProxyApp(
    {
      ...
      upstreamHandler: async (request) => {
        ...
        if (auth === "Bearer REDACTED_SECRET") {
          return {
            status: 403,
            headers: {
              "content-type": "application/json"
    ...
    async ({ app }) => {
      ...
      assert.equal(response.headers["x-open-hax-upstream-provider"], "vivgrid");
      assert.ok(observedAuth.length >= 2);
      assert.deepEqual(observedAuth.slice(-2), ["REDACTED_SECRET", "vivgrid-working-key"]);
...
test("falls back from ollama-cloud to vivgrid when gpt model is missing on ollama", async () => {
  ...
  await withProxyApp(
    ...
    async ({ app }) => {
      ...
      assert.equal(response.headers["x-open-hax-upstream-provider"], "vivgrid");
      assert.ok(observedAuth.length >= 2);
      assert.deepEqual(observedAuth.slice(-2), ["REDACTED_SECRET", "vivgrid-gpt-key"]);
...
test("fails over streamed chat-completions accounts when requested reasoning trace is missing", async () => {
  ...
  await withProxyApp(
    {
      keys: ["REDACTED_SECRET", "key-kimi-stream-with-reasoning"],
      upstreamHandler: async (request) => {
        const auth = request.headers.authorization;
        if (typeof auth === "string") {
          observedKeys.push(auth.replace(/^Bearer\s+/i, ""));
    ...
    async ({ app }) => {
      ...
      assert.deepEqual(observedKeys, ["REDACTED_SECRET", "key-kimi-stream-with-reasoning"]);
