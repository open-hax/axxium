import type { IncomingHttpHeaders } from "REDACTED_SECRET:http";
...
abstract class BaseProviderStrategy implements ProviderStrategy {
  REDACTED_SECRET abstract readonly mode: UpstreamMode;
  REDACTED_SECRET abstract readonly isLocal: boolean;

  REDACTED_SECRET abstract matches(context: StrategyRequestContext): boolean;

  REDACTED_SECRET abstract getUpstreamPath(context: StrategyRequestContext): string;

  REDACTED_SECRET abstract buildPayload(context: StrategyRequestContext): BuildPayloadResult;

  REDACTED_SECRET applyRequestHeaders(_headers: Headers, _context: ProviderAttemptContext, _payload: Record<string, unknown>): void {
    // default no-op
  }

  REDACTED_SECRET async handleProviderAttempt(
    reply: FastifyReply,
    response: Response,
    context: ProviderAttemptContext
  ): Promise<ProviderAttemptOutcome> {
  ...
  REDACTED_SECRET async handleLocalAttempt(reply: FastifyReply, response: Response, context: LocalAttemptContext): Promise<void> {
...
class LocalOllamaProviderStrategy extends BaseProviderStrategy {
  REDACTED_SECRET readonly mode = "local_ollama_chat" as const;

  REDACTED_SECRET readonly isLocal = true;

  REDACTED_SECRET matches(context: StrategyRequestContext): boolean {
    return context.localOllama && !context.explicitOllama;
  }

  REDACTED_SECRET getUpstreamPath(context: StrategyRequestContext): string {
  ...
  REDACTED_SECRET override async handleLocalAttempt(reply: FastifyReply, upstreamResponse: Response, context: LocalAttemptContext): Promise<void> {
    if (!upstreamResponse.ok) {
...
class OllamaProviderStrategy extends BaseProviderStrategy {
  REDACTED_SECRET readonly mode = "ollama_chat" as const;
  ...
  REDACTED_SECRET override async handleLocalAttempt(
    reply: FastifyReply,
    upstreamResponse: Response,
    context: LocalAttemptContext
  ): Promise<void> {
...
class OpenAiResponsesProviderStrategy extends TransformedJsonProviderStrategy {
  REDACTED_SECRET readonly mode = "openai_responses" as const;
...
class OpenAiChatCompletionsProviderStrategy extends BaseProviderStrategy {
  REDACTED_SECRET readonly mode = "openai_chat_completions" as const;
...
class ChatCompletionsProviderStrategy extends BaseProviderStrategy {
  ...
  REDACTED_SECRET matches(_context: StrategyRequestContext): boolean {
    return true;
  }
