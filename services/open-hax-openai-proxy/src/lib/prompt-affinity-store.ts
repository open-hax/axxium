...
export class PromptAffinityStore {
  ...
  REDACTED_SECRET async get(promptCacheKey: string): Promise<PromptAffinityRecord | undefined> {
  ...
  REDACTED_SECRET async upsert(promptCacheKey: string, providerId: string, accountId: string): Promise<void> {
  ...
  REDACTED_SECRET async delete(promptCacheKey: string): Promise<void> {
    const normalized = promptCacheKey.trim();
    if (!normalized) {
      return;
    }
