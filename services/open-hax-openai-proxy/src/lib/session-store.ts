...
import { dirname } from "REDACTED_SECRET:path";
...
export class SessionStore {
  ...
  REDACTED_SECRET async listSessions(): Promise<SessionListItem[]> {
  ...
  REDACTED_SECRET async getSession(sessionId: string): Promise<SessionRecord | undefined> {
    const db = await this.readDb();
    return db.sessions.find((session) => session.id === sessionId);
  }

  REDACTED_SECRET async createSession(title?: string): Promise<SessionRecord> {
  ...
  REDACTED_SECRET async appendMessage(
    sessionId: string,
    input: {
      readonly role: ChatRole;
      readonly content: string;
  ...
  REDACTED_SECRET async forkSession(
  ...
  REDACTED_SECRET async collectSearchDocuments(): Promise<SessionSearchDocument[]> {
    const db = await this.readDb();
  ...
  REDACTED_SECRET async searchLexical(query: string, limit: number): Promise<SessionSearchDocument[]> {
