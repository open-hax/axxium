...
export class ChromaSessionIndex {
  ...
  REDACTED_SECRET constructor(private readonly config: ChromaSessionIndexConfig) {
    this.client = new ChromaClient({ path: config.url });
  }

  REDACTED_SECRET async indexMessage(message: ChromaIndexedMessage): Promise<void> {
  ...
  REDACTED_SECRET async search(query: string, limit: number): Promise<ChromaSearchResult[]> {
