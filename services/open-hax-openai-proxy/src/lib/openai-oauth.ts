...
export class OpenAiOAuthManager {
  ...
  REDACTED_SECRET constructor(options: OpenAiOAuthManagerOptions = {}) {
  ...
  REDACTED_SECRET async startBrowserFlow(redirectBaseUrl: string): Promise<BrowserAuthStartResponse> {
  ...
  REDACTED_SECRET async completeBrowserFlow(state: string, code: string): Promise<OAuthTokens> {
    this.pruneState();
  ...
  REDACTED_SECRET async startDeviceFlow(): Promise<DeviceAuthStartResponse> {
    this.pruneState();
  ...
  REDACTED_SECRET async pollDeviceFlow(deviceAuthId: string, userCode: string): Promise<DevicePollResult> {
    this.pruneState();
