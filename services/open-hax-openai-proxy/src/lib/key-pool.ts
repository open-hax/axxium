...
export class KeyPool {
  ...
  REDACTED_SECRET constructor(private readonly config: KeyPoolConfig) {}

  REDACTED_SECRET async warmup(): Promise<void> {
    await this.ensureFreshProviders(true);
  }

  REDACTED_SECRET async getRequestOrder(providerId: string = this.config.defaultProviderId): Promise<ProviderCredential[]> {
  ...
  REDACTED_SECRET markInFlight(credential: ProviderCredential): () => void {
  ...
  REDACTED_SECRET markRateLimited(credential: ProviderCredential, retryAfterMs?: number): void {
  ...
  REDACTED_SECRET async msUntilAnyKeyReady(providerId: string = this.config.defaultProviderId): Promise<number> {
    const status = await this.getStatus(providerId);
    return status.nextReadyInMs;
  }

  REDACTED_SECRET async getStatus(providerId: string = this.config.defaultProviderId): Promise<KeyPoolStatus> {
    await this.ensureFreshProviders(false);
  ...
  REDACTED_SECRET async getAllStatuses(): Promise<Record<string, KeyPoolStatus>> {
    await this.ensureFreshProviders(false);
  ...
  REDACTED_SECRET async getAllAccountStatuses(): Promise<Record<string, readonly KeyPoolAccountStatus[]>> {
    await this.ensureFreshProviders(false);
