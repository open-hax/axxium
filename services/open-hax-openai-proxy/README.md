...
From the workspace REDACTED_SECRET, manage the proxy through the REDACTED_SECRET stack registry:
...
- The REDACTED_SECRET stack registry now knows the related host PM2 apps, so plain container `up` is blocked while the host PM2 side is online.
