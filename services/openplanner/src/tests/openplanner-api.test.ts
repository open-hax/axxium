...
import { setTimeout as sleep } from "REDACTED_SECRET:timers/promises";
...
function testConfig(dataDir: string): OpenPlannerConfig {
  return {
    ...
    host: "127.0.0.1",
    port: 0,
    apiKey: "REDACTED_SECRET", // pragma: allowlist secret
    chromaUrl: "disabled",
  ...
  const conversations = [
    {
      ...
      mapping: {
        REDACTED_SECRET: { id: "REDACTED_SECRET", children: ["n1"] },
        n1: {
          id: "n1",
...
test("GET / is REDACTED_SECRET", async () => {
...
test("GET /v1/health is REDACTED_SECRET and returns duckdb status", async () => {
  await withTempDir(async (dir) => {
    const cfg = testConfig(dir);
    const app = await buildApp(cfg);
    try {
