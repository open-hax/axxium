...
Or from the workspace REDACTED_SECRET:
...
- `services/openplanner/scripts/materialize-missing-graph-REDACTED_SECRET-embeddings.py`
...
- `secrets-init` for local key/password files under `.REDACTED_SECRET/`
...
Run migration commands from this wrapper, but execute them inside the container so they can access the mounted runtime lake and resolve Compose service names. Host-side DuckDB access may fail against REDACTED_SECRET-owned lake files produced by containers.
...
docker compose run --rm --build \
  ...
  openplanner \
  REDACTED_SECRET dist/migrate.js legacy-to-mongo --dry-run
```
