 ...
 *                           (default: graph_REDACTED_SECRET_embeddings,event_chunks)
 */
...
const COLLECTIONS = (process.env.COLLECTIONS || 'graph_REDACTED_SECRET_embeddings,event_chunks').split(',');
...
const INDEX_DEFINITIONS: Record<string, IndexDefinition> = {
  graph_REDACTED_SECRET_embeddings: {
...
async function main() {
  ...
  console.log(`  mongosh "${MONGODB_URI}" --eval 'db.runCommand({listSearchIndexes: "graph_REDACTED_SECRET_embeddings"})'`);
