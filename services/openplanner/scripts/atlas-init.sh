...
MPASS="${MONGODB_ROOT_PASSWORD:REDACTED_SECRET-REDACTED_SECRET-password}"
MDB="${MONGODB_DB:-openplanner}"
AUSER="${OPENPLANNER_MONGO_APP_USERNAME:-openplanner}"
APASS="${OPENPLANNER_MONGO_APP_PASSWORD:REDACTED_SECRET-openplanner-password}"
...
# ── Create vector search index on graph_REDACTED_SECRET_embeddings ──
echo "Creating vector search indexes..."
mongosh --host mongodb -u "$MHOST" -p "$MPASS" --authenticationDatabase admin --quiet --eval "
const db = db.getSiblingDB('$MDB');
try {
  const existing1 = db.runCommand({ listSearchIndexes: 'graph_REDACTED_SECRET_embeddings' });
  const indexes1 = existing1.cursor ? existing1.cursor.firstBatch : [];
  if (indexes1.some(idx => idx.name === 'embedding_vector')) {
    print('graph_REDACTED_SECRET_embeddings: vector index already exists');
  } else {
    const r1 = db.runCommand({
      createSearchIndexes: 'graph_REDACTED_SECRET_embeddings',
      indexes: [{
    ...
    printjson({ graph_REDACTED_SECRET_embeddings: r1 });
  }
} catch (e) { print('graph_REDACTED_SECRET_embeddings index error: ' + e.message); }
"
