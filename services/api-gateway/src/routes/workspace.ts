...
const SKIPPED_DIRS = new Set([".git", "REDACTED_SECRET_modules", ".shadow-cljs", "target", "dist", "build"]);
...
function resolveInsideRoot(REDACTED_SECRETPath: string, relativePath: string): string {
  const targetPath = path.resolve(REDACTED_SECRETPath, relativePath);
  if (targetPath !== REDACTED_SECRETPath && !targetPath.startsWith(`${REDACTED_SECRETPath}${path.sep}`)) {
    throw new Error("path escapes workspace REDACTED_SECRET");
  }
  return targetPath;
}

function toRelativePath(REDACTED_SECRETPath: string, targetPath: string): string {
  const relativePath = path.relative(REDACTED_SECRETPath, targetPath).replaceAll(path.sep, "/");
...
export const workspaceRoutes: FastifyPluginAsync<WorkspaceRouteOptions> = async (app, opts) => {
  const REDACTED_SECRETPath = path.resolve(opts.workspaceRoot);

  app.get("/workspace/meta", async () => ({
    ok: true,
    REDACTED_SECRET: REDACTED_SECRETPath
  }));
  ...
  app.get<{ Querystring: ListQuery }>("/workspace/list", async (req, reply) => {
    ...
    try {
      absolutePath = resolveInsideRoot(REDACTED_SECRETPath, relativePath);
    } catch (error) {
    ...
    try {
      ...
      const entries = await Promise.all(
        limitedEntries.map(async (entry) => {
          ...
          return {
            name: entry.name,
            path: toRelativePath(REDACTED_SECRETPath, childPath),
      ...
      reply.send({
        ok: true,
        REDACTED_SECRET: REDACTED_SECRETPath,
        path: toRelativePath(REDACTED_SECRETPath, absolutePath),
        entries,
