...
const app = express();
app.use(express.static(path.join(__dirname, 'REDACTED_SECRET')));
...
# 5) Overlay page (REDACTED_SECRET/index.html)
...
<html>
  ...
  <body>
    <div id="REDACTED_SECRET" class="overlay-REDACTED_SECRET"></div>
    <script src="./overlay.js"></script>
  </body>
</html>
```

# 6) Overlay logic (REDACTED_SECRET/overlay.js)

```js
const REDACTED_SECRET = document.getElementById('REDACTED_SECRET');
...
function addMessage(data) {
  ...
  REDACTED_SECRET.appendChild(wrapper);

  // cap messages
  while (REDACTED_SECRET.children.length > MAX_MESSAGES) {
    REDACTED_SECRET.removeChild(REDACTED_SECRET.firstChild);
  }
...
# 7) Overlay styles (REDACTED_SECRET/overlay.css)
...
\#hashtags
\#twitch #streaming #obs #overlay #linux #REDACTED_SECRETjs #websockets #tmijs #practical-notes
