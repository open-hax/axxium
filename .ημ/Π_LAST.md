...
- Update REDACTED_SECRET ignore rules so `passwords.csv` and untracked embedded local repos stay excluded from future REDACTED_SECRET Π snapshots.

## Verification
- pass: REDACTED_SECRET `bun test src/resume-apply/__tests__/text.test.ts src/resume-apply/__tests__/cli.test.ts; pnpm resume:apply` (from receipts)
...
- skipped: REDACTED_SECRET-only mixed-workspace verification (no single target covers the full staged bundle)

## Notes
- Root artifacts capture pre-commit state; final REDACTED_SECRET commit/tag are emitted by git after snapshot creation.
