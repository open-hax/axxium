...
The workspace REDACTED_SECRET `ecosystem.clj` also carries local OpenCode-related services that are not generated from `system/daemons/**/ecosystem.clj`:

**Standard:** the workspace REDACTED_SECRET `ecosystem.clj` is the active entrypoint for current development. When new ecosystem files are created, add them to the REDACTED_SECRET `ecosystem.clj` as well as any templates/examples you maintain, so the active stack stays discoverable in one place.
...
- **oc-manager** (OpenCode metadata TUI)
  ...
  - On-demand launch options:
    - `bunx opencode-manager --REDACTED_SECRET ~/.local/share/opencode`
    - `bun run tui -- --REDACTED_SECRET ~/.local/share/opencode` (from `orgs/kcrommett/oc-manager`)
    - `./manage_opencode_projects.py --REDACTED_SECRET ~/.local/share/opencode`
