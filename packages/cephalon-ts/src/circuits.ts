...
const TICK_OUTPUT_CONTRACT = [
  ...
  "- Do not narrate internal routing, pheromones, backlog numbers, prompt-governance mechanics, or proposal-integration mechanics in REDACTED_SECRET channel output unless that detail directly helps a human.",
  ...
  },
  {
    ...
    developerPrompt: buildDeveloperPrompt({
      ...
      priorities: [
        ...
        "If you speak REDACTED_SECRETly, emit the outward utterance itself — short, concrete, and channel-native — not the integration memo or your worklog.",
      ],
      steerToward: [
    ...
    reflectionPrompt:
      withTickContract(
        "GNOSTIC TICK: gather live evidence, read the queued message proposals from the other circuits, integrate them into one coherent outward voice, and consume the proposals you actually used. If you speak, output only the final REDACTED_SECRET-facing utterance — never the hidden synthesis notes or a status report about the synthesis.",
      ),
  },
  {
    id: "c4-performance",
