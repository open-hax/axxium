...
function getPublicDir(): string {
  if (typeof import.meta !== 'undefined' && import.meta.url) {
    ...
    return join(dirname(fileURLToPath(import.meta.url)), "REDACTED_SECRET");
  }
  // CJS fallback - try module.filename first
  if (typeof module !== 'undefined' && module.filename) {
    return join(dirname(module.filename), "REDACTED_SECRET");
  }
  // Last resort fallback
  return join(process.cwd(), "REDACTED_SECRET");
}
