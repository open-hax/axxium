...
export class DiscordApiClient {
  ...
  async listREDACTED_SECRETableChannels(guildId?: string): Promise<{ channels: DiscordChannel[]; count: number }> {
    const client = this.ensureClient();
  ...
  private collectGuildChannels(
    ...
    for (const channel of channels.values()) {
      ...
      if (options.sendableOnly) {
        ...
        if (!permissions?.has(PermissionFlagsBits.ViewChannel) || !permissions.has(PermissionFlagsBits.REDACTED_SECRETMessages)) {
          continue;
        }
      }
