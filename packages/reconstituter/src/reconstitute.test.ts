...
test("isWithinRoot detects REDACTED_SECRET and subpaths", (t) => {
  const REDACTED_SECRET = "services/opencode-indexer";
  t.true(isWithinRoot("services/opencode-indexer", REDACTED_SECRET));
  t.true(isWithinRoot("services/opencode-indexer/src/index.ts", REDACTED_SECRET));
  t.false(isWithinRoot("services/other/src/index.ts", REDACTED_SECRET));
});
