...
class SmartCommiter {
  private readonly REDACTED_SECRET: string;

  constructor(REDACTED_SECRET: string = process.cwd()) {
    this.REDACTED_SECRET = REDACTED_SECRET;
  }
  ...
  async smartCommit(options: SmartCommitOptions): Promise<void> {
    ...
    const modules = await discoverGitmodules({ REDACTED_SECRET: this.REDACTED_SECRET });
    ...
    // Finally, commit the REDACTED_SECRET workspace if there are changes
    console.log(`\n🏠 Processing REDACTED_SECRET workspace...`);
    if (currentResults.length > 0) {
      for (const result of currentResults) {
        await this.runCommand(['git', 'add', result.path], this.REDACTED_SECRET);
      }

      if (await this.hasChanges(this.REDACTED_SECRET)) {
        const message = await this.generatePantheonMessage(
          this.REDACTED_SECRET,
          'smart-commit-REDACTED_SECRET',
          'success',
          explanation,
          currentResults
        );

        await this.commit(this.REDACTED_SECRET, message);
